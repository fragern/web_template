...
class C_karyawan extends CI_Controller {
	...
	public function Index()
	{
		...
		// // $this->load->view('karyawan/V_karyawan',$data); // Tampil data
        $this->load->view('karyawan/V_karyawan'); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_karyawan";         
        $search = array('hdrid','nik','nama','jenis_kelamin','foto','tanggal_masuk','alamat');
         // jika memakai IS NULL pada where sql
         $isWhere = null;
         // $isWhere = 'artikel.deleted_at IS NULL';
         header('Content-Type: application/json');
         echo $this->M_karyawan->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_karyawan->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_karyawan->Max_data($mdate,'tb_karyawan')->row();        
        ...
        $this->M_karyawan->Input_Data($post_datamerge,'tb_karyawan');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_karyawan');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_karyawan->Update_Data($where,$post_datamerge,'tb_karyawan');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_karyawan->ajax_getbyhdrid($hdrid,'tb_karyawan')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_karyawan->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_karyawan->get_requester($id_user_reg); 
		$data['data']=$this->M_karyawan->get_data($id_user_reg); 
        
        $this->load->view('email/V_karyawan',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_karyawan->Update_Data($where,$data_update,$table);
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				foreach ($sheetData as $key => $value) {
                    ...
                    //  Sample
                        ...
                        $resultData[$index]['foto'] = ucwords($value['A']);
                        $resultData[$index]['tanggal_masuk'] = ucwords($value['A']);
                        $resultData[$index]['alamat'] = ucwords($value['A']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_karyawan->insert_batch('tb_karyawan',$resultData);
					if ($result > 0) {
