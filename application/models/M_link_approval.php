...
class M_link_approval extends CI_Model {
   ...
   function ajax_getbyPartNo($PartNo,$table){      
      return $this->db->get_where($table, array('PartNo' => $PartNo));
   }
   ...
   public function Send_mail($PartNo)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM visitor_approver
     where PartNo='WTR2010009' and level_approve=(select min(level_approve) from visitor_approver where PartNo='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       $where = array('PartNo' => $query->PartNo); 
       $data = array('progress_approver' =>'Waiting approved '.$query->name_approver);       
       $this->db->where($where);
       $this->db->update('visitor_approver',$data);

      // Kirim email request to approve
      $this->db->query("CALL sp_send_mail @PartNo='$query->PartNo',@nik='$query->nik_approver',@name='$query->name_approver',@email='widodo.a5v@ap.denso.com',@EmailSubject='Waiting Approver Wire transfer No ',@Email_body_content='You need approver'");
     ...
     $this->db->query("exec sp_send_mail @PartNo='123',@nik='DM1902060',@name='Widodo',@email='widodo.a5v@ap.denso.com',@EmailSubject='Finish Approver Wire transfer No ',@Email_body_content='You need approver'");
   
    }
    
   }

   public function Send_mail_reject($PartNo)
   {
    
      // Cari data wire
      $query =$this->db->query("select PartNo,nik,name_requester,isnull(reject_by,'')reject_by,isnull(reason_reject,'')reason_reject from tb_wiretransfer where PartNo='$PartNo' ");
      $query=$query->row();
      ...
      if ($query2->num_rows()>0) {
          $query2=$query2->row();
         //  Kirim email
          $result =$this->db->query("exec sp_send_mail @PartNo='$query->PartNo',@nik='$query->nik',@name='$query->name_requester',@email='widodo.a5v@ap.denso.com',@EmailSubject='Rejected Wire transfer No ',@Email_body_content='Rejected by <b> $query->reject_by </b>',@comment='With reason = <b> $query->reason_reject </b>'")->result_array();
      }
   ...
   public function ajax_getbyIsSection()
    {
        $this->db->select('*');
        $this->db->from('tb_is_section');
        return $this->db->get();
    }

    public function ajax_get_approval_section($section_code)
    {

        $query =$this->db->query("select * from tb_is_section where kode_department='$section_code'");
        return $query;
    ...
    function cari_tb_approver($hdrid)
    {
      ...
      $query = $this->db->query("SELECT top 1 * from tb_approval where hdrid='$hdrid' and date_approve is null order by sequence asc");
      if ($query->num_rows() > 0) {
          return $query->row();
      }else{
          $query = (object) array('nik'=>'not found','name'=>'All','office_email'=>'not found','sequence'=>'not found');
          return $query;
      }
    ...
    //Kepakai
    function cari_count_tb_approver($hdrid)
    {

      $query = $this->db->query("SELECT top 1 * from tb_approval where hdrid='$hdrid' and date_approve is null order by sequence desc");
      if ($query->num_rows() > 0) {
          return $query->row();
      }else{
          $query = (object) array('nik'=>'not found','name'=>'All','office_email'=>'not found','sequence'=>'not found');
   ...
   function Update_Data_Approve($where,$data,$table,$hdrid){
        ...
        $sql="SELECT TOP 1 * FROM tb_approval WHERE hdrid ='$hdrid' AND date_approve IS NULL order by  sequence asc";
        $query = $this->db->query($sql);
        if ($query->num_rows()>0) {
            $query=$query->row();
            $status_approval = "Visitor need approval by ".$query->name." (".$query->department_name." )";
            $this->db->query("UPDATE tb_visitor SET status_transaction ='$status_approval' where hdrid='$hdrid' ");

        }else{
            $status_approval="Done";
            $this->db->query("UPDATE tb_visitor SET status_transaction ='Done' where hdrid='$hdrid' ");
        }
    ...
    function reject_Data_Approve($hdrid){
        ...
        $this->db->query("UPDATE tb_approval SET date_approve=NULL where hdrid='$hdrid'");
    ...
    function cari_inisiator_responsible($hdrid)
    {
      ...
      if ($query->num_rows() > 0) {
          ...
          $query = (object) array('nik'=>'not found','name'=>'not found','office_email'=>'not found','sequence'=>'not found');
          return $query;
      }
